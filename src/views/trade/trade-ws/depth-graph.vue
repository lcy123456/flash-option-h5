...
// 监听深度数据
watch(
	[() => tradeStore.state.echartsData],
	() => {
		const eData = tradeStore.state.echartsData
		const asksArr = eData.ask?.items
			?.sort((a: any, b: any) => parseFloat(a.amount) - parseFloat(b.amount))
			.map((item) => {
				return [item.price, item.amount]
			})
		state.echartsData.asksArr = asksArr
		option.yAxis[0].min = Math.min(eData.ask?.minAmount, eData.bid?.minAmount)
		option.yAxis[0].max = Math.min(eData.ask?.maxAmount, eData.bid?.maxAmount)
		const buysArr = eData.bid?.items
