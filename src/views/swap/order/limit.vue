...
import { cutPostion } from '@/apis/swap'
...
const closeHold = async () => {
	const params = {
		...
		profitLossPrice: state.formInfo.profitLossPrice, // 委托价格
		volume: inputVol.value // 委托数量
	}
	if (!isVail(params)) return
	const res: any = await cutPostion(params)
	const isSuccess = res.code === 0
	if (isSuccess) {
		swapStore.fetchPostionList()
	}
	isSuccess ? showSuccessToast(t('trade.txt58')) : showFailToast(t('trade.txt59'))
...
const estimatedProft = computed(() => {
	...
	const price = state.formInfo.profitLossPrice
...
const profitRate = computed(() => {
	...
	if (!profitAmount || !state.formInfo.profitLossPrice) return '--'
	...
	<component
		...
		<div class="mt-5 mb-3 px-4">
			<div class="flex items-center justify-between mb-[22px]" v-if="!isPC">
				<div class="text-text-primary text-style-web-h3-bold">{{ t('trade.txt60') }}</div>
				<div class="">
			...
			<NtIInput
				...
				:readonly="true"
				class="mb-3"
				:right-text="t('trade.txt65')" />
			<NtIInput
				:label="t('trade.txt45')"
				:i-placeholder="t('trade.txt66')"
				v-model="state.formInfo.profitLossPrice"
		...
		<div class="flex items-center border-t border-border-strong py-5 px-4">
			<Button
				type="small"
