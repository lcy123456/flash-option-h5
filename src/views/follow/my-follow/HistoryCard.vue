...
import { historyFollowOrder } from '@/apis/follow'
...
const columns = reactive([
	...
	},
	{
		...
		key: 'rewardAmount',
		render(item: any) {
			return h('span', { class: colorChange(item?.rewardAmount) }, item?.rewardAmount + 'USDT')
		}
	},
	{
		title: t('flw.txt102'),
...
const updateList = async () => {
	loading.value = true
	const res: any = await historyFollowOrder({
		...
		<ListView v-model:loading="loading" v-model:finished="finished" @load="onLoad" class="pt-0">
			<div v-for="item in list" :key="item.id" class="p-4 border-b border-border-subtle">
				<div class="pb-2 flex justify-between">
					...
					<div class="text-text-disabled text-style-web-base-medium">{{ t('flw.txt44') }}</div>
				</div>
