        ...
        this._send(this._datafeedURL + "/history", {
            ...
            .done((response) => {
                ...
                for (let i = 0; i < data.length; i++) {
                    const item = data[i];
                    bars.push({
                        time: isValidMillisecondTimestamp(item[0]),
                        open: item[1],
                        high: item[2],
                        low: item[3],
                        close: item[4],
                ...
                // bars = bars.filter((item: Bar) => item.time !== -1)
...
function isValidMillisecondTimestamp(timestamp) {
...
const formatTime = (time: any, format = 'YYYY-MM-DD HH:mm:ss') => {
	if (!time) return '--'
